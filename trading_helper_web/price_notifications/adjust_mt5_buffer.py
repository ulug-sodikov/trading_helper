...
def adjust_mt5_buffer():
    ...
    response = requests.get('http://metatrader5_api_service:8080/symbols_buffer')
    mt5_symbols_buffer = set(response.json()['symbols_buffer'])

    for symbol in symbols.difference(mt5_symbols_buffer):
        requests.patch(f'http://metatrader5_api_service:8080/symbols_buffer/{symbol}')
    ...
    # XAUUSD ticks are used in frontend.
    ghost_symbols.discard('XAUUSD')
    for symbol in ghost_symbols:
        requests.delete(f'http://metatrader5_api_service:8080/symbols_buffer/{symbol}')
